...
@fast.agent(
    instruction="You are a web automation testing agent. Use Playwright MCP server to interact with web app. Execute test tasks provided using BDD/Gherkin format. Always take screenshot at the end of each bdd scenario and store it as {filename}.png under ./artifacts directory. Before and after each bdd scenario start browser_start_tracing and stop browser_stop_tracing then save it to ./artifacts/{filename}/ directories. Test report should be in the markdown format and saved as {filename}.md in the ./artifacts directory",
    servers=["playwright"]
)
